...
public interface SearchCardRepository extends JpaRepository<SearchCard, Long> {
    ...
    @Query("""
            update SearchCard card
            set card.status = com.dasifind.backend.domain.searchcard.model.SearchCardStatus.EXPIRED,
                card.closedAt = card.searchExpiresAt,
                card.closeReason = null,
                card.updatedAt = :now
            where card.status = com.dasifind.backend.domain.searchcard.model.SearchCardStatus.ACTIVE
                and card.searchExpiresAt < :now
            """)
    int expireDueCards(@Param("now") LocalDateTime now);
