...
class ApiResDTOTest {
    ...
    void result를_포함한_성공_응답을_생성한다() {
        ApiResDTO<String> response = ApiResDTO.success("result");
    ...
    void result가_null인_실패_응답을_생성한다() {
        ApiResDTO<Void> response = ApiResDTO.failure(ErrorCode.RESOURCE_NOT_FOUND);
    ...
    void 명세에_정의된_필드명으로_JSON을_생성한다() {
        String json = jsonMapper.writeValueAsString(ApiResDTO.success("result"));
