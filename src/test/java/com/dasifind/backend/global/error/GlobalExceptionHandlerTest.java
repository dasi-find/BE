...
class GlobalExceptionHandlerTest {
    ...
    void 비즈니스_예외를_정의된_오류_응답으로_변환한다() {
        ResponseEntity<ApiResDTO<Void>> response = exceptionHandler.handleBusinessException(
                new BusinessException(ErrorCode.EMAIL_ALREADY_EXISTS)
        );

        assertThat(response.getStatusCode().value()).isEqualTo(409);
        assertThat(response.getBody()).isEqualTo(ApiResDTO.failure(ErrorCode.EMAIL_ALREADY_EXISTS));
    }

    @Test
    void 올바르지_않은_요청을_공통_400_응답으로_변환한다() {
        ResponseEntity<ApiResDTO<Void>> response = exceptionHandler.handleInvalidRequest(
                new HttpMessageNotReadableException("invalid", new MockHttpInputMessage(new byte[0]))
        );

        assertThat(response.getStatusCode().value()).isEqualTo(400);
        assertThat(response.getBody()).isEqualTo(ApiResDTO.failure(ErrorCode.INVALID_REQUEST));
    }
    ...
    void 필수값_검증에_실패하면_필수_입력값_누락으로_응답한다() {
        ...
        ResponseEntity<ApiResDTO<Void>> response = exceptionHandler.handleMethodArgumentNotValid(exception);

        assertThat(response.getStatusCode().value()).isEqualTo(400);
        assertThat(response.getBody()).isEqualTo(ApiResDTO.failure(ErrorCode.REQUIRED_FIELD_MISSING));
    }
    ...
    void multipart_최대_크기를_초과하면_이미지_용량_오류로_응답한다() {
        ResponseEntity<ApiResDTO<Void>> response = exceptionHandler.handleMaxUploadSizeExceeded(
                new MaxUploadSizeExceededException(10L * 1024 * 1024)
        );

        assertThat(response.getStatusCode().value()).isEqualTo(413);
        assertThat(response.getBody()).isEqualTo(ApiResDTO.failure(ErrorCode.IMAGE_TOO_LARGE));
    }

    @Test
    void 예상하지_못한_예외의_내부_내용을_응답에_노출하지_않는다() {
        ResponseEntity<ApiResDTO<Void>> response = exceptionHandler.handleUnexpectedException(
                new RuntimeException("sensitive detail")
        );

        assertThat(response.getStatusCode().value()).isEqualTo(500);
        assertThat(response.getBody()).isEqualTo(ApiResDTO.failure(ErrorCode.INTERNAL_SERVER_ERROR));
